...
export const GenesisPage: React.FC = () => {
  ...
  // Auto-clear image generation error after 5 seconds
  useEffect(() => {
    if (error) {
  ...
  const handleToggleRecording = useCallback(() => {
    ...
    recognition.onresult = (event) => {
      ...
      if (error) setError(null); // Clear general error on voice input as well
    };
      ...
      <div className="w-full p-6 space-y-6 overflow-y-auto border-b lg:w-2/5 xl:w-1/3 lg:border-b-0 lg:border-r border-meebot-border">
        ...
        <div className="space-y-6">
            ...
            <div>
              ...
              {speechError && <p className="mt-2 text-xs text-red-400 animate-fade-in">{speechError}</p>}
            </div>
