...
export const MeeBotProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToMiner(USER_WALLET_ADDRESS, (remoteData) => {
        if (remoteData) {
            setMiningState(prev => {
                // Determine if we need to update state to match remote
                // We use lastMinedAt as the source of truth for synchronization
  ...
  const executeMining = useCallback(async () => {
      ...
      setMiningState(prev => {
          ...
          // --- SYNC TO LEADERBOARD & FIRESTORE ---
          const activeBot = meebots[0];
          if (activeBot) {
             // Pass the precise timestamp to ensure consistency
             updateMinerScore(
                USER_WALLET_ADDRESS,
                activeBot.name,
                newPoints,
